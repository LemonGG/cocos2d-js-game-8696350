...
lc.getIsBookor()
lc.generalSetting = function(){
	var l =window.document.getElementById("gameico")
	l.href = 'res/'+lc.language+'/favicon.ico';
	l.type = '';
	l.type = 'image/GIF';
}
lc.generalSetting()
...
lc.point = {
	getWorldPoint: function (display) {
		return display.parent.convertToWorldSpace(display.getPosition());
	},
...
lc.collision = {
	d2d : function(display1,display2){
		if(!display1.parent)return
		if(!display2.parent)return
		var p1 = display1.parent.convertToWorldSpace(display1.getPosition());
		var p2 = display2.parent.convertToWorldSpace(display2.getPosition());
		var rect1 = display1.getBoundingBox()//cc.rect(p1.x,p1.y,tar1.width,tar1.height)//tar1.getBoundingBox();
		var rect2 = display2.getBoundingBox()//cc.rect(p2.x,p2.y,tar2.width,tar2.height)//tar2.getBoundingBox();
		rect1.x = p1.x;
		rect1.y = p1.y;
		rect2.x = p2.x;
		rect2.y = p2.y;
